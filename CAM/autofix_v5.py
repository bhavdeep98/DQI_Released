...
class NLI_infer_BERT(nn.Module):
    def __init__(self,
        ...
        #self.model = RobertaForSequenceClassification.from_pretrained(pretrained_dir, num_labels=3).cuda()
        self.model = RobertaForSequenceClassification.from_pretrained(pretrained_dir, num_labels=3)
...
class NLIDataset_BERT(Dataset):
    ...
    def __init__(self,
        ...
        self.tokenizer = RobertaTokenizer.from_pretrained(pretrained_dir, do_lower_case=True, add_special_tokens=True) 
...
def attack(premise, hypothese, true_label, predictor, stop_words_set, word2idx, idx2word, cos_sim, sim_predictor=None,
    ...
    if true_label != orig_label:
        ...
        for idx, score in sorted(enumerate(import_scores), key=lambda x: x[1], reverse=True):
            if score > import_score_threshold and hypothese[idx] not in stop_words_set and doc[idx].pos_ not in ['NOUN','PROPN','NUM']:
        ...
        for idx, synonyms in tqdm(synonyms_all):
            ...
            sim_list = []
            for hyp in new_texts:
                #print(' '.join(premise))
                sim_list.append(abs(((nlp(' '.join(premise))).similarity(nlp(' '.join(hyp))))))
            ...
            new_probs_mask = []
            # we need same label
            print("This is the orignial label")
            print(orig_label)
            for prob,sim in zip(new_probs,sim_list):
                element = torch.argmax(prob, dim=-1)
                if element != orig_label:
                    p_list = sorted(prob.tolist()[:])
                    #print(p_list)
                    max_1 = p_list[-1]
                    max_2 = p_list[-2]
                    if abs(max_1-max_2)<0.2:## HYPER-PARAMETER
                        if sim<=0.8:## HYPER-PARAMETER
                            new_probs_mask.append(0)
                        else:
                            new_probs_mask.append(1)
                    else:
                        new_probs_mask.append(1)
                else:
                    new_probs_mask.append(1)
...
def main():
    ...
    parser.add_argument("--target_model",
                        type=str,
                        required=True,
                        choices=['infersent', 'esim', 'bert', 'roberta'],
                        help="Target models for text classification: fasttext, charcnn, word level lstm "
                             "For NLI: InferSent, ESIM, bert-base-uncased, roberta-large")

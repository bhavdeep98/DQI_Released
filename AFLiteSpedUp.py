...
if EMBEDDING_BOOL:
    ...
    for index in tqdm(range(int(start),len(sentences_list))):
        if index%50==0:
            ...
            X_prime = []
            ## overwrite the index so far in seprate file                                                with open("/scratch/srmishr1/Results/RFLITE/SNLI/Roberta_1e_5_32_final/start_pooled_1.data","w") as fileHandle:
                fileHandle.write(str(index))
        #print(index)
        try:
    #     input_ids = torch.tensor(tokenizer.encode(S['sentence'][index], add_special_tokens=True)).unsqueeze(0)  # Batch size 1                                                                      input_ids = torch.tensor(tokenizer.encode(sentences_list[index], add_special_tokens=True, max_length=512,truncation=True)).unsqueeze(0)
...
with open('/scratch/srmishr1/Results/RFLITE/SNLI/Roberta_1e_5_32_final/listfile_pooled_1.data', mode='b+r') as f:
    while 1:                                                                                         try:
            X.append(pickle.load(f))                                                                 except EOFError as error:
            # Output expected EOFErrors.
            break
        except Exception as exception:                                                                   # Output unexpected Exceptions.
